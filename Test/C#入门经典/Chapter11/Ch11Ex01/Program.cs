...
{ 
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //foreach循环，把animalArray的数放到myAnimal
            foreach (Animal myAnimal in animalArray)
            {
                Console.WriteLine("New {0} object added to Array collection, " + "Name = {1}", myAnimal.ToString(), myAnimal.Name);
            }

            Console.WriteLine("Array collectiom contains {0} objects.", animalArray.Length);
            animalArray[0].Feed();  //类型是Animal的数组，Feed()是类Animal的方法
            ((Chicken)animalArray[1]).LayEgg();  //类Animal派生类的方法，不能直接调用，需要强制转换
            Console.WriteLine();

            Console.WriteLine("Create an ArrayList type collection of Animal " + "object and ude it");
